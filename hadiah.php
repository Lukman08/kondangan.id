...
if(isset($_POST['bsimpan']))
{
	...
	{
		//Data akan di edit
		$edit = mysqli_query($conn, "UPDATE hadiah set
								nama = '$_POST[tnama]',
								hadiah = '$_POST[thadiah]',
	...
	else
	{
		...
		$simpan = mysqli_query($conn, "INSERT INTO hadiah (nama, hadiah, alamat)
		VALUES ('$_POST[tnama]', 
				'$_POST[thadiah]', 
				'$_POST[talamat]')
				");
...
if(isset($_GET['hal']))
{
	...
	if($_GET['hal'] == "edit")
	{
		...
		$tampil = mysqli_query($conn, "SELECT * FROM hadiah WHERE id = '$_GET[id]' ");
	...
	else if ($_GET['hal'] == "hapus")
	{
		...
		$hapus = mysqli_query($conn, "DELETE FROM hadiah WHERE id = '$_GET[id]' ");
		if($hapus){
			echo "<script>
...
<div class="card mt-3">
	...
	<form method="post" action="">
		...
		<div class="form-group">
			<label>Hadiah</label>
			<input type="text" name="thadiah" value="<?=@$vhadiah?>" class="form-control" placeholder="Input Hadiah" required>
		</div>
		<div class="form-group">
			<label>Alamat</label>
		...
		<button class="btn btn-warning btn"><a href="cetak_hadiah.php" target="_blank">Cetak</a></button>
	</form>
</div>
</div>
<!-- Akhir Card Form -->
...
<div class="card mt-3">
	...
	<table class="table table-bordered table-striped">
		...
		<?php
			$no = 1;
			$tampil = mysqli_query($conn, "SELECT * from hadiah");
		...
		<tr>
			...
			<td><?=$data['alamat']?></td>
			<td>
				<a href="hadiah.php?hal=edit&id=<?=$data['id']?>" class="btn btn-warning"> Edit </a>
				<a href="hadiah.php?hal=hapus&id=<?=$data['id']?>" 
