...
if(isset($_POST['bsimpan']))
{
	...
	{
		//Data akan di edit
		$edit = mysqli_query($conn, "UPDATE kehadiran set
											 nama = '$_POST[tnama]',
											 waktu = '$_POST[twaktu]',
		...
		if($edit) //jika edit sukses
		{
			echo "<script>
					...
					document.location='kehadiran.php';
				 </script>";
		}
		else
		{
	...
	else
	{
		...
		$simpan = mysqli_query($conn, "INSERT INTO kehadiran (nama, waktu, alamat)
		VALUES ('$_POST[tnama]', 
				 '$_POST[twaktu]', 
				 '$_POST[talamat]')
	   ");
...
if(isset($_GET['hal']))
{
	...
	if($_GET['hal'] == "edit")
	{
		...
		$tampil = mysqli_query($conn, "SELECT * FROM kehadiran WHERE id = '$_GET[id]' ");
	...
	else if ($_GET['hal'] == "hapus")
	{
		...
		$hapus = mysqli_query($conn, "DELETE FROM kehadiran WHERE id = '$_GET[id]' ");
		if($hapus){
			echo "<script>
...
<div class="card mt-3">
  ...
  <div class="card-body">
	<form method="post" action="">
		...
		<div class="form-group">
			<label></label>
			<input type="time" name="twaktu" value="<?=@$vwaktu?>" class="form-control" placeholder="Input Jumlah" required>
		</div>
		<div class="form-group">
			<label>Alamat</label>
		...
		<button class="btn btn-warning btn"><a href="cetak_kehadiran.php" target="_blank">Cetak</a></button>
	</form>
  </div>
</div>
<!-- Akhir Card Form -->
...
<div class="card mt-3">
  ...
  <div class="card-body">
	...
	<table class="table table-bordered table-striped">
		...
		<?php
			$no = 1;
			$tampil = mysqli_query($conn, "SELECT * from kehadiran");
		...
		<tr>
			...
			<td><?=$data['alamat']?></td>
			<td>
				<a href="kehadiran.php?hal=edit&id=<?=$data['id']?>" class="btn btn-warning"> Edit </a>
				<a href="kehadiran.php?hal=hapus&id=<?=$data['id']?>" 
