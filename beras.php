...
if(isset($_POST['bsimpan']))
{
	...
	{
		//Data akan di edit
		$edit = mysqli_query($conn, "UPDATE beras set
										nama = '$_POST[tnama]',
	...
	else
	{
		...
		$simpan = mysqli_query($conn, "INSERT INTO beras (nama, jumlah, alamat)
		VALUES ('$_POST[tnama]', 
					'$_POST[tjumlah]', 
					'$_POST[talamat]')
					");
			...
			if(isset($_GET['hal']))
			{
				...
				$tampil = mysqli_query($conn, "SELECT * FROM beras WHERE id = '$_GET[id]' ");
				...
				$hapus = mysqli_query($conn, "DELETE FROM beras WHERE id = '$_GET[id]' ");
				if($hapus){
					echo "<script>
...
<div class="card mt-3">
	...
	<div class="card-body">
		<table class="table table-bordered table-striped">
			...
			<?php
			$no = 1;
			$tampil = mysqli_query($conn, "SELECT * from beras");
			...
			<tr>
				...
				<td><?=$data['alamat']?></td>
				<td>
					<a href="beras.php?hal=edit&id=<?=$data['id']?>" class="btn btn-warning"> Edit </a>
					<a href="beras.php?hal=hapus&id=<?=$data['id']?>" 
